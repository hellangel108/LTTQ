...
namespace Stelia_BUS
{
    public class Stelia_BUS
    {
        ...
        #endregion
        #region Search
        public DataTable timKiemNhanhDiemDanh(string date)
        {
            return dd.timKiemDiemDanh_NhanVien("", date);
        }
        ...
        public string QuetMa_SP(string mavach)
        {
            string rep="";
            DataTable infotable = qm.getQuetMa(mavach);
            DataRowCollection result = infotable.Rows;
            foreach (DataRow row in result)
                    rep= row[0].ToString();
            return rep;
