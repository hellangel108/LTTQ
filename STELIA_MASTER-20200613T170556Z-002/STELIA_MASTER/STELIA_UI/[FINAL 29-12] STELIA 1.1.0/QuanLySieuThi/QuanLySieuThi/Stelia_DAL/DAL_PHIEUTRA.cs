...
namespace Stelia_DAL
{
    public class DAL_PHIEUTRA: DAL_connect
    {
        ...
        public DataTable timKiemtheoMaPhieuTra(string mapt)
        {
            SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM PHIEUTRA " +
                                                   "WHERE MAPT LIKE '" + mapt + "' ", connect);
            DataTable dtPhieuTra = new DataTable();
            da.Fill(dtPhieuTra);
            return dtPhieuTra;
        }
        public DataTable timKiemChiTiet(string mapt, string mancc)
        {
            SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM PHIEUTRA " +
                                                   "WHERE MAPT LIKE '%" + mapt + "%' AND MANCC LIKE '%" + mancc + "%'", connect);
