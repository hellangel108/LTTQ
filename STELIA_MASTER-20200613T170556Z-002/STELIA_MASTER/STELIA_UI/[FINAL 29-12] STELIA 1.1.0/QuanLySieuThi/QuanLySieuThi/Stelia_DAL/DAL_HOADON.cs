...
namespace Stelia_DAL
{
    public class DAL_HOADON : DAL_connect
    {
        ...
        {
            SqlDataAdapter da = new SqlDataAdapter("SELECT SUM(THANHTIEN) FROM HOADON " +
                                                   "WHERE NGHD = '" + ngay + "' " +
                                                   "GROUP BY NGHD", connect);
            DataTable data = new DataTable();
            da.Fill(data);
            return data;
        }
        ...
        {
            SqlDataAdapter da = new SqlDataAdapter("SELECT SUM(THANHTIEN) FROM HOADON " +
                                                   "WHERE MONTH(NGHD) = '" + th + "' AND YEAR(NGHD) = '" + nm + "' " +
                                                   "GROUP BY MONTH(NGHD)", connect);
            DataTable data = new DataTable();
            da.Fill(data);
            return data;
        }
        ...
        {
            SqlDataAdapter da = new SqlDataAdapter("SELECT COUNT(THANHTIEN) FROM HOADON " +
                                                   "WHERE NGHD = '" + ngay + "' " +
                                                   "GROUP BY NGHD", connect);
            DataTable data = new DataTable();
            da.Fill(data);
